...
export function einsteinLlmGateway(modelName: string, headers?: Record<string, string | undefined>): LanguageModel {
    return {
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            try {
              ...
              const content: string | undefined = jsonData.generation_details?.generations?.[0]?.content;
